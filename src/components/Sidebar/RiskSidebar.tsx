...
interface SidebarProps { isOpen: boolean; setIsOpen: React.Dispatch<React.SetStateAction<boolean>> }
...
<button type="button" aria-label="메뉴 열기" onClick={() => setIsOpen(true)} className="rounded-md focus:outline-none text-[#333333] hover:text-[#FF3300] transition-colors duration-300">
<Bars3Icon className="w-6 h-6" />
</button>
<div className="text-[15px] font-semibold mx-auto text-[#333333]">위험성평가</div>
</header>
)}
{isOpen && <div className="fixed inset-0 bg-black bg-opacity-40 z-40 md:hidden" />}
...
<NavLink to={path} onClick={() => setIsOpen(false)} className={({ isActive }) => `group flex items-center gap-1 py-1.5 px-2 rounded-lg transition-colors duration-300 hover:bg-[#FFF1F0] hover:text-[#FF3300] ${isActive ? "bg-[#FFF1F0] text-[#FF3300]" : "text-[#5C5E6D]"}`}>
<span className="truncate">{label}</span>
</NavLink>
</li>
{idx === 0 && (<li className="py-2"><hr className="border-t border-[#DDDDDD]" /></li>)}
</React.Fragment>
))}
</ul>
</div>
