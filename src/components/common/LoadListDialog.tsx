...
export default function LoadListDialog({
  ...
  const data: DataRow[] = items.map((item, idx) => ({
    id: sampleDates[idx] ?? sampleDates[0],
    title: item.name,
  }))
  ...
  return (
    <>
      ...
      >
        ...
        <div className="text-red-600 text-right mb-4 text-sm">
          * 완료된, '승인완료' 상태의 위험성평가표를 불러옵니다.
        </div>
        <style>{`
          .no-checkbox th:first-child,
          .no-checkbox td:first-child { display: none; }
        `}</style>
        <div className="flex-grow overflow-y-auto no-checkbox">
          <DataTable
            ...
            renderCell={(row, col) => (
              <span
                onClick={() =>
                  toggleSelect(col.key === "title" ? String(row.title) : row.id)
                }
                className="block px-2 py-1 transition-colors duration-200 hover:text-[#1C56D3] cursor-pointer"
                style={{ fontSize: 15 }}
              >
