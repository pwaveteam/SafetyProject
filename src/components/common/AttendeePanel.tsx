...
function PhoneInput({ phonePrefix, phoneMiddle, phoneLast, onChangePrefix, onChangeMiddle, onChangeLast }: { phonePrefix: string; phoneMiddle: string; phoneLast: string; onChangePrefix: (value: string) => void; onChangeMiddle: (value: string) => void; onChangeLast: (value: string) => void }) {
...
<select name="phonePrefix" value={phonePrefix} onChange={e => onChangePrefix(e.target.value)} className={`${BASE_INPUT} pr-8`}>
<option value="010">010</option><option value="070">070</option>
</select>
...
<input type="text" name="phoneMiddle" value={phoneMiddle} onChange={e => onChangeMiddle(e.target.value)} maxLength={4} className={`${BASE_INPUT} basis-1/3`} onKeyDown={e => { if (!/[0-9]/.test(e.key) && !["Backspace", "ArrowLeft", "ArrowRight", "Delete", "Tab"].includes(e.key)) e.preventDefault() }} />
<span className="text-[15px] text-[#333639]">-</span>
<input type="text" name="phoneLast" value={phoneLast} onChange={e => onChangeLast(e.target.value)} maxLength={4} className={`${BASE_INPUT} basis-1/3`} onKeyDown={e => { if (!/[0-9]/.test(e.key) && !["Backspace", "ArrowLeft", "ArrowRight", "Delete", "Tab"].includes(e.key)) e.preventDefault() }} />
</div>
)}
...
{ key: "action", label: "삭제", minWidth: 60, renderCell: (_, __, idx) => (<button onClick={() => onRemove(idx)}><X size={16} /></button>) }
]
...
if (!phoneMiddle.trim() || !phoneLast.trim()) { alert("전화번호를 정확히 입력해주세요"); return }
onAdd({ name, phone: fullPhone })
setName(""); setPhonePrefix("010"); setPhoneMiddle(""); setPhoneLast("")
}

const exampleLoadItems: Item[] = [
{ id: 1, registrationDate: "2025-07-20", siteName: "서울시 공사현장 A", tbmName: "TBM-01", attendeeCount: 6, attendeeList: "홍길동, 이안전, 김근로, 최근로, 박안전, 최안전" },
{ id: 2, registrationDate: "2025-07-18", siteName: "부산항 재개발 현장", tbmName: "TBM-02", attendeeCount: 5, attendeeList: "홍길동, 김근로, 박안전, 최안전, 최근로" },
{ id: 3, registrationDate: "2025-07-15", siteName: "대전 신도시 조성 현장", tbmName: "TBM-03", attendeeCount: 6, attendeeList: "이안전, 김근로, 최근로, 박안전, 최안전, 홍길동" }
]

const openLoadDialog = () => { setLoadItems(exampleLoadItems); setIsLoadDialogOpen(true) }
const openDeptGroupDialog = () => { setIsDeptGroupDialogOpen(true) }
const onSelectLoadItem = (selected: string | number | null) => { if (selected !== null) alert(`선택된 TBM 내역 ID: ${selected}`) }
const onSelectDeptGroupItem = (selected: string | number | null) => { if (selected !== null) alert(`선택된 부서 그룹 ID: ${selected}`) }
...
<input type="text" placeholder="이름을 입력해주세요" value={name} onChange={e => setName(e.target.value)} maxLength={20} className={BASE_INPUT} onKeyDown={e => { if (!/[a-zA-Zㄱ-ㅎㅏ-ㅣ가-힣]/.test(e.key) && !["Backspace", "ArrowLeft", "ArrowRight", "Delete", "Tab"].includes(e.key)) e.preventDefault() }} onPaste={e => e.preventDefault()} />
<PhoneInput phonePrefix={phonePrefix} phoneMiddle={phoneMiddle} phoneLast={phoneLast} onChangePrefix={setPhonePrefix} onChangeMiddle={setPhoneMiddle} onChangeLast={setPhoneLast} />
</div>
...
<LoadListDialogTBM isOpen={isLoadDialogOpen} items={loadItems} onChangeSelected={onSelectLoadItem} onClose={() => setIsLoadDialogOpen(false)} />
<LoadListDialogGroup isOpen={isDeptGroupDialogOpen} items={deptGroupItems} onChangeSelected={onSelectDeptGroupItem} onClose={() => setIsDeptGroupDialogOpen(false)} />
</div>
)
}