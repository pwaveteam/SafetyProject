...
const Breadcrumb: React.FC = () => {
  ...
  if (pathnames.length === 1 && pathnames[0] === "safety-education") {
    crumbs.push({
      url: "/safety-education/regular",
      name: pathNameMap["/safety-education/regular"],
    })
  }
  ...
  return (
    <nav
      ...
      className="flex items-center gap-1 mb-3 whitespace-nowrap text-[13px] leading-tight"
    >
      <button
        onClick={() => navigate("/")}
      ...
      {crumbs.map((crumb, idx) => (
        <React.Fragment key={`${crumb.url}-${idx}`}>
          ...
          <span
            ...
            style={{ color: "#999999", userSelect: "none", fontSize: 13 }}
          >
            {crumb.name}
          </span>
        </React.Fragment>
