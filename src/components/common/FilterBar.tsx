...
const INPUT_CLASS =
  "h-[36px] border border-[#AAAAAA] rounded-[8px] px-3 bg-white focus:outline-none focus:ring-2 focus:ring-[#B9D0F6] text-sm font-normal text-[#333639] placeholder:text-[#AAAAAA]"
...
const FilterBar: React.FC<FilterBarProps> = ({
  ...
  const methodOptions = [
    ...
    { value: "현장교육", label: "현장교육" },
    { value: "자체교육", label: "자체교육" },
    { value: "외부전문가교육", label: "외부전문가교육" },
  ]
    ...
    <section className="tbm-filter w-full flex flex-wrap items-center gap-3 px-3 py-3 mb-3 bg-[#F8F8F8] border border-[#E5E5E5] rounded-[10px]">
      <div className="flex flex-wrap items-center gap-3 flex-grow min-w-0">
          ...
          <div className="flex gap-3 flex-shrink-0 w-full sm:w-auto">
            <div className="relative flex items-center min-w-[160px]">
              ...
              <select
                ...
                onChange={e => onEducationTargetChange(e.target.value)}
              >
                {targetOptions.map(opt => (
                  <option key={opt.value} value={opt.value} className="text-sm font-normal text-[#333639]">
                    {opt.label}
                  </option>
                ))}
              </select>
            ...
            <div className="relative flex items-center min-w-[160px]">
              ...
              <select
                ...
                onChange={e => onEducationMethodChange(e.target.value)}
              >
          ...
          <>
            {educationTarget !== undefined && onEducationTargetChange && (
              <div className="relative flex items-center flex-shrink-0 w-full sm:w-auto min-w-[160px]">
                <span className={LABEL_CLASS}>교육대상</span>
                <select
                  className={`${INPUT_CLASS} w-full sm:w-[160px] appearance-none pr-8`}
                  value={educationTarget}
                  onChange={e => onEducationTargetChange(e.target.value)}
                >
                  {targetOptions.map(opt => (
        ...
        {supervisor !== undefined && onSupervisor && (
          <div className="relative flex items-center flex-shrink-0 w-full sm:w-auto min-w-[160px]">
            ...
            >
              <option value="" className="text-sm font-normal text-[#333639]">
                -전체-
              </option>
              <option value="이동현" className="text-sm font-normal text-[#333639]">
                이동현
              </option>
            </select>
            <ChevronDown className="absolute right-2 pointer-events-none text-gray-500" />
        ...
        {(keyword !== undefined && onKeywordChange) || (searchText !== undefined && onSearchText) ? (
          <div className="relative flex items-center flex-shrink-0 w-full sm:w-auto min-w-[280px]">
            ...
            <input
              type="text"
              className={`${INPUT_CLASS} w-full sm:w-[280px] pr-10`}
              placeholder="검색어 입력"
      ...
      <div className="w-full sm:w-auto flex justify-end">
        <Button
          ...
          onClick={onSearch}
          style={{ height: 36, minWidth: 80, fontSize: "0.875rem" }}
        >
          검색
        </Button>
      </div>
