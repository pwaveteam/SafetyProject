...
return (
<section
className="w-full h-28 rounded-[16px] relative text-white p-6 flex items-center border border-[#cccccc]"
aria-label="Banner"
style={{
background: `linear-gradient(135deg, #333333 20%, #5A5F63 80%)`,
}}
>
<div className="flex flex-col justify-center space-y-1 w-full">
<h4 className="font-medium text-sm sm:text-lg text-left">{banners[index].title}</h4>
<p className="font-medium text-xs sm:text-sm text-left">{banners[index].desc}</p>
<nav className="flex space-x-2 mt-2" aria-label="Banner indicators">
{banners.map((_, idx) => (
<span
key={idx}
className="rounded-full w-2 h-2 transition-colors duration-300"
style={{
backgroundColor: index === idx ? "white" : "rgba(255, 255, 255, 0.4)",
}}
/>
))}
</nav>
