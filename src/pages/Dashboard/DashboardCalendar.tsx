...
if (e.endDate) {
return e.startDate <= dateStr && dateStr <= e.endDate
}
...
type="button"
onClick={prevMonth}
className="p-1 border rounded hover:bg-gray-100 absolute left-0"
aria-label="이전 달"
...
{currentDate.getFullYear()}년 {getKoreanMonth(currentDate.getMonth())}
</span>
<button
type="button"
onClick={todayClick}
className="text-[12px] font-semibold px-3 rounded-full border"
style={{
color: MAIN_COLOR,
...
type="button"
onClick={nextMonth}
className="p-1 border rounded hover:bg-gray-100 absolute right-0"
aria-label="다음 달"
style={{ borderWidth: BORDER_WIDTH }}
>
...
<div
className="grid grid-cols-7 text-center text-[#161616]"
style={{
borderTop: `${BORDER_WIDTH}px solid ${BORDER_COLOR}`,
borderLeft: `${BORDER_WIDTH}px solid ${BORDER_COLOR}`,
...
style={{
borderRight: i === 6 ? "none" : `${BORDER_WIDTH}px solid ${BORDER_COLOR}`,
userSelect: "none",
}}
>
<span className={`${day === "일" ? "text-red-600" : ""} ${day === "토" ? "text-blue-900" : ""}`}>
{day}
</span>
</div>
))}
...
<div
key={i}
className={`h-[66px] flex flex-col p-2 overflow-hidden ${
!isCurMonth ? "bg-[#F8F8F8] text-gray-400" : "bg-white"
}`}
style={{
borderTop: `${BORDER_WIDTH}px solid ${BORDER_COLOR}`,
borderRight: isLastCol ? undefined : `${BORDER_WIDTH}px solid ${BORDER_COLOR}`,
boxSizing: "border-box",
}}
title={[...singleEvents, ...periodEvents].map((e) => e.title).join(", ")}
>
<div
...
? "text-gray-400"
: isTodayFlag
? "text-blue-600 bg-blue-100 inline-flex items-center justify-center"
: dayOfWeek === 0
? "text-red-600"
: dayOfWeek === 6
? "text-blue-900"
...
style={{
fontSize: "1rem",
width: isTodayFlag ? 30 : undefined,
height: isTodayFlag ? 29 : undefined,
borderRadius: isTodayFlag ? "50%" : undefined,
padding: isTodayFlag ? "3px" : undefined,
minWidth: isTodayFlag ? 29 : undefined,
textAlign: isTodayFlag ? "center" : undefined,
}}
>
{day.getDate()}
</div>
...
<div
key={event.id}
className="leading-tight truncate flex items-center"
title={event.title + (event.time ? ` (${event.time})` : "")}
style={{ whiteSpace: "nowrap", color: "#333333", fontSize: "11px" }}
>
<span
style={{
...
height: 6,
borderRadius: "50%",
backgroundColor: event.title.includes("점검")
? "#031E36"
: event.title.includes("교육")
? "#3D9EFF"
: "#FF3300",
marginRight: 4,
flexShrink: 0,
...
<div
key={`period-dot-${event.id}-${day.toISOString().slice(0, 10)}`}
className="leading-tight truncate flex items-center"
title={`${event.title} (기간: ${event.startDate} ~ ${event.endDate})`}
style={{ whiteSpace: "nowrap", color: "#333333", fontSize: "11px" }}
>
<span
style={{
