// src/screens/TBMRegisterScreen.tsx
...
export default function TBMRegisterScreen() {
  ...
  const toggleExpand = (proc: string) =>
    setExpandedProcesses(prev => ({ ...prev, [proc]: !prev[proc] }))
  ...
  const handleRiskItemChange = (
    proc: string,
    index: number,
    key: "hazard" | "countermeasure",
    value: string
  ) => {
    setProcessDetails(prev => {
  ...
  const fields: Field[] = [
    ...
    { label: "파일첨부", name: "fileUpload", type: "fileUpload" },
  ]
  ...
  const handleAddAttendee = (att: Attendee) => {
    setAttendeesList(prev => [...prev, att])
  }
