...
export default function AssetManagement() {
  ...
  const handleDelete = () => {
    ...
    if (window.confirm("정말 삭제하시겠습니까?")) {
      if (currentTabIdx === 0) {
        setMachineData(prev => prev.filter(row => !checkedIds.includes(row.id)))
      } else {
        setHazardData(prev => prev.filter(row => !checkedIds.includes(row.id)))
      }
      setCheckedIds([])
    }
  }

  const handleSafetyApplication = () => {
    alert("안전검사 신청서 버튼 클릭됨")
  }
