...
{
id: 3,
content: "기계실 바닥이 미끄러워 미끄럼 방지 매트 설치가 필요",
registrant: "김근로",
date: "2025-06-01",
status: "조치",
reason: "외부 승인 대기 중",
...
{
id: 2,
content: "출입구에 비상 연락처 QR 코드를 부착해 비상 시 신속한 대응 지원 바람",
registrant: "익명",
date: "2025-06-01",
status: "조치",
reason: "외부 승인 대기 중",
...
{
id: 1,
content: "고소 작업 안전난간 간격이 넓어 추가 난간 또는 보조 안전로프 설치 요청",
registrant: "익명",
date: "2025-06-01",
status: "미조치",
reason: "추가 조사 검토 필요",
...
<button
type="button"
onClick={() => handleClick("조치")}
className={`mr-1 px-1 py-1 rounded font-semibold cursor-pointer ${
isSelected("조치") ? "text-green-600" : "text-gray-300"
}`}
aria-pressed={isSelected("조치")}
>
<Badge color="green" className={isSelected("조치") ? "" : "opacity-30"}>
조치
</Badge>
</button>
<button
type="button"
onClick={() => handleClick("미조치")}
className={`px-1 py-1 rounded font-semibold cursor-pointer ${
isSelected("미조치") ? "text-orange-500" : "text-gray-300"
}`}
aria-pressed={isSelected("미조치")}
>
<Badge color="orange" className={isSelected("미조치") ? "" : "opacity-30"}>
미조치
</Badge>
</button>
</div>
...
placeholder="미조치 사유 입력"
disabled={row.status === "조치"}
style={{
width: "96%",
padding: "8px 8px",
borderRadius: 8,
...
fontSize: "0.875rem",
backgroundColor: row.status === "조치" ? "#f3f3f3" : "white",
cursor: row.status === "조치" ? "not-allowed" : "auto",
}}
/>
</div>
)
